...
var autoCheckout = true;
var checkout_delay = 6000;
...
async function mobileAPIRefreshed(respond) {
  if (
    respond['products_and_categories'] == null ||
    respond['products_and_categories']['new'] == null
  ) {
    return false;
  }
  ...
  for (let index = 0; index < newProducts.length; index++) {
    ...
    if (
      item != null &&
      item['name'] != null &&
      matchKeyWord(item['name'], keyWord)
    ) {
      isNew = true;
      return true;
    }
...
async function monitor() {
  ...
  if (refreshed) {
    ...
    startTime = new Date();
    console.log(
      'Detect Page refreshed with mobile endpoint at: ' +
        startTime.toISOString()
    );
    window.location.href = isNew
      ? 'https://www.supremenewyork.com/mobile/#categories/new'
      : 'https://www.supremenewyork.com/mobile/#categories/' + category;
...
async function addToCart() {
  if (
    document.getElementById('cart-update').children[0].innerHTML === 'remove'
  ) {
    checkout();
    return;
  }
...
async function waitTillCheckoutPageIsOpen() {
  ...
  if (checkoutBtn) {
    ...
    setTimeout(async function() {
      await waitTillCheckoutPageIsOpen();
    }, 200);
